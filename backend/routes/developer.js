...
router.get(
  "/dashboard",
  authenticateToken,
  authenticateRole(ROLE.DEVELOPER),
  async (req, res) => {
    try {
    ...
    try {
      ...
      if (old_password && new_password) {
        const [old] = await pool.query(
          `SELECT password_hash FROM developer WHERE dev_id = ?`,
          [dev_id],
        );
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const { studio_name, email, bio, currency, language, password } =
        req.body;
      ...
      const query = `INSERT INTO developer(user_id, studio_name, email, bio, currency_id, language_id, password_hash)
        VALUES(?, ?, ?, ?, ?, ?, ?)`;
      const [devRows] = await pool.query(query, [
        req.user.id,
        studio_name,
        email,
        bio,
    ...
    try {
      ...
      try {
        ...
        await connection.query(
          `DELETE g, dg
            FROM games g
            JOIN dev_games dg ON g.game_id = dg.game_id
            WHERE dg.dev_id = ?
            `,
          [dev_id],
        );

        await connection.query("DELETE FROM developer WHERE dev_id = ?", [
          dev_id,
        ]);
        ...
        res.json({
          success: true,
          message: "Developer account and related games deleted successfully",
        });
      } catch (err) {
